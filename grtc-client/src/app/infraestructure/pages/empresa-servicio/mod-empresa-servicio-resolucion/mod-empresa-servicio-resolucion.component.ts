...
export class ModEmpresaServicioResolucionComponent implements OnInit {
  ...
  GuardarDatos() {
    const erroresValidacion = crear_empresa_servicio_resolucion_vf(
      this.dataResolucion
    );
  ...
  CrearResolucionEmpresaServicio() {
    this.resolucionService
      .CrearResolucionEmpresaServicio(this.dataEmpresaServicioResolucion)
      .subscribe({
        next: (data: any) => {
          console.log(data);
        },
        ...
        complete: () => {
          ...
          this.router.navigate([
            'principal/detalle-empresa-servicio/' +
              this.dataEmpresaServicioResolucion.id_empresa_servicio,
          ]);
