...
class PersonaController{
    ...
    public async ObtenerPersonaBydocumento(req: Request, res: Response): Promise<void> {
        try {
            const { documento } = req.params;
            const consulta= 'select * from t_persona where documento = $1';
            const persona = await db.query(consulta,[documento]);
    ...
    public async EliminarPersona(req: Request, res: Response): Promise<void> {
        try {
            ...
            const consulta = `
                ...
                `;
            db.query(consulta, [id_persona], (error) => {
                if (error) {
